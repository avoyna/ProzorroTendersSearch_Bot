...
def select_interesting_prozorro(db_filename, last_viewed_tender_id=None, start_date=None, end_date=None):
    ...
    try:
        ...
        statement = """SELECT t.internal_ID, tl.id, t.title, t.value_amount, 
            ...
            i.deliveryAddress_locality, i.deliveryAddress_streetAddress
                FROM Tender t
                LEFT JOIN items i ON i.tender_internal_ID = t.internal_ID
                LEFT JOIN procuringEntity pe ON pe.internal_ID = t.procuringEntity_internal_ID
        ...
        if start_date:
            ...
            AND DATE(t.dateModified) <= ?"""
                ...
                value = (start_date,)
            statement += """
                ORDER BY Tender_list_internal_ID ASC;"""
        ...
        elif last_viewed_tender_id:
            statement += """
            AND t.dateModified >= ?"""
            value = (last_viewed_tender_id, )
            statement += """
                ORDER BY Tender_list_internal_ID ASC;"""
            cursor_obj.execute(statement, value)
        else:
